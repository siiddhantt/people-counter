...
def run_voice_assistant():
    # Check if the VoiceAssistant process is already running
    for proc in subprocess.Popen("ps aux | grep VoiceAssistant.py | grep -v grep", shell=True, stdout=subprocess.PIPE).stdout:
        if "VoiceAssistant.py" in proc.decode():
            print('VoiceAssistant is already running')
            return
    # If VoiceAssistant process is not running, start it as a new process
    path = os.path.join(os.getcwd(), 'VoiceAssistant.py')
    subprocess.Popen(['python', path])
    print('VoiceAssistant has been started as a new process')


def kill_voice_assistant():
    # Check if the VoiceAssistant process is already running
    for proc in psutil.process_iter():
        if proc.name() == 'python' and 'VoiceAssistant.py' in ' '.join(proc.cmdline()):
            print(f"Terminating process {proc.pid} - VoiceAssistant.py")
            proc.kill()
            return
            ...
            if cy in range(up_limit, down_limit):
                for i in persons:
                    if abs(x-i.getX()) <= w and abs(y-i.getY()) <= h:
                        ...
                        new = False
                        # actualiza coordenadas en el objeto and resets age
